...
class TranscriptionApp:
    ...
    def clean_text(self, text):
        """Cleans up the text by removing repeated words and unnecessary filler phrases."""
    ...
    def transcribe_audio(self):
        ...
        with MicrophoneStream(RATE, CHUNK) as stream:
            ...
            for response in responses:
                ...
                # Only update if the cleaned text is different from the last
                if clean_text != self.last_transcript:
                    self.last_transcript = clean_text
                    self.transcription_text.delete(1.0, tk.END)
                    self.transcription_text.insert(tk.END, self.last_transcript)
